...
char selected_fxs[3] = {0,0,0};
unsigned char selected_fx_unit = 0;

unsigned char fx_unit_amount[4] = {0,0,0,0};

typedef enum FX_UNIT_MODES {
    UNIT_MODE_GROUP = 0,
    UNIT_MODE_SINGLE = 1
} fxunittype_t;

unsigned char selected_fx_unit_mode = UNIT_MODE_GROUP;

unsigned char fx_unit_states[4] = {STATE_OFF,STATE_OFF,STATE_OFF,STATE_OFF};
...
unsigned char getFxModeFromTraktorValue(unsigned char value) {
    return value == 0 ? UNIT_MODE_GROUP : UNIT_MODE_SINGLE;
}
...
void handleCC(unsigned char evnt1, unsigned char evnt2) {
    ...
    if ( evnt1 == 3 ) {
        ...
        // Effect unit mode
        selected_fx_unit_mode = getFxModeFromTraktorValue(evnt2);
        
    } else if ( evnt1 >= 5 && evnt1 <= 8 ) {
        // Dry wet adjust
        fx_unit_amount[evnt1 - 5] = evnt2;
        
    } else if ( evnt1 >= 9 && evnt1 <= 12 ) {
        // Effect unit state (on/off)
        fx_unit_states[evnt1 - 9] = getFxStateFromTraktorValue(evnt2);
        ...
        selected_fxs[fx_number] = evnt2 != 0 ? selected_fx + 1 : 0;
 
    } else if ( evnt1 == 17 || evnt1 == 21 ) {
        // Multicolor DIN control (beat jump & loop leds)
        pin = evnt2 / 25 * 3;
        start = evnt1 == 17 ? 0 : 16;
...
void printEffectName(unsigned char fxNumber) {
    MIOS_LCD_PrintCString(effect_names[fxNumber]);
}

void moveCursorToRow(unsigned char row) {
    MIOS_LCD_CursorSet(getOffsetForRow(row));
}
...
void DISPLAY_Tick(void) __wparam
{
    unsigned char i = 0;
    unsigned char j = 0;
    unsigned char first_fx_unit_on_display = selected_fx_unit > 2 ? 1 : 0;
    unsigned char fx_unit_index = 0;
    ...
    for (i = 0; i <= 2; ++i) {
        fx_unit_index = i + first_fx_unit_on_display;
        moveCursorToRow(i);
        
        if (selected_fx_unit == fx_unit_index) {
            MIOS_LCD_PrintChar(126); // ->
        } else {
            MIOS_LCD_PrintChar(' ');
        }
        
        MIOS_LCD_PrintBCD1(fx_unit_index + 1);
        
        if (fx_unit_states[fx_unit_index] == STATE_ON) {
            MIOS_LCD_PrintChar(165); // *
        } else {
            MIOS_LCD_PrintChar(' ');
        }
        
        for (j = 0; j <= 6; ++j) {
            if (fx_unit_amount[fx_unit_index] == 127 ||
                fx_unit_amount[fx_unit_index] >= (j + 1) * 21) {
                MIOS_LCD_PrintChar(219); // []
            } else {
                MIOS_LCD_PrintChar(' ');
            }
        }
    ...
    for (i = 0; i <= 2; ++i) {
        ...
        if ( selected_fx_unit_mode == UNIT_MODE_GROUP || i == 0 ) {
            printEffectName(selected_fxs[i]);
        } else {
            printEffectName(0);
        }
    }
